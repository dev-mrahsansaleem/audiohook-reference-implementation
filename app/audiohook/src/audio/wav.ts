    ...
    static async create(filename: string, format: AudioFormat, rate: number, channels: number): Promise<WavFileWriter> {
        ...
        dataWriter.on('finish', () => {
            ...
            if (writer.format === 'PCMU') {
                writer.header.writeUInt32LE(numSamples, 40);                // fact chunk value (number of samples)
...
export interface WavReader {
    ...
    close(): Promise<void>;
    skip(samples: number): Promise<number>;
...
class WavFileReader implements WavReader {
    ...
    constructor(file: FileHandle, startPos: number, dataChunkSize: number, format: AudioFormat, rate: SampleRate, channels: number) {
        ...
        this.totalSamples = dataChunkSize/this.bytesPerSample;
    }
    ...
    async skip(samples: number): Promise<number> {
        const available = Math.min(
            this.dataChunkSize - this.readPos, 
            (samples <= 0) ? 0 : Math.trunc(samples * this.bytesPerSample)
        );
        this.readPos += available;
        return available/this.bytesPerSample;
    }

    async readNext(samples: number): Promise<Uint8Array | Int16Array | null> {
        const available = Math.min(
            Math.trunc(samples <= 0 ? 0 : this.bytesPerSample*Math.min(samples, this.maxFrameSamples)),
            this.dataChunkSize - this.readPos, 
        );
        if (available === 0) {
            return null;
        }
        const position = this.dataStartPos + this.readPos;
        this.readPos += available;
        const buffer = this.bufferFactory(available);
        const res = await this.file.read({ buffer, position });
